...
public class FireSwords extends ItemSword{
        ...
        if(!worldIn.isRemote){
            ...
            if (player.hasCapability(SlayerProvider.Breath_CAP, null)) {
                if (player.getCapability(SlayerProvider.Breath_CAP, null).getBreath() == 1) {
                    if(player.getCapability(TechProvider.TECH_CAP, null).getTech() == 1 && mana>20){
                        worldIn.playSound(player, player.posX, player.posY, player.posZ,  SoundHandler.SIRANUI_SOUND, SoundCategory.AMBIENT, 1f, 1f);
                    ...
                    if(player.getCapability(TechProvider.TECH_CAP, null).getTech() == 2 && mana > 25){
                        if(tick==0) {
                            tick = 30;
                            enten_flag = true;
                            worldIn.playSound(player, player.posX, player.posY, player.posZ,  SoundHandler.ENTEN_SOUND, SoundCategory.AMBIENT, 1f, 1f);
                    ...
                    if(player.getCapability(TechProvider.TECH_CAP, null).getTech() == 5 && mana > 40){
                        if(tick==0) {
                            tick = 40;
                            enko_flag = true;
                    ...
                    if(player.getCapability(TechProvider.TECH_CAP, null).getTech() == 9 && mana > 60){
                        if(tick==0) {
                            tick = 70;
                            rengoku_flag = true;
                            player.getCapability(SlayerProvider.Breath_CAP, null).setMana(mana - 60.0F);
                            Networking.sendTo(new Slayer(player.getCapability(SlayerProvider.Breath_CAP, null)), (EntityPlayerMP) player );
                        }
                    }
                    if(player.getCapability(TechProvider.TECH_CAP, null).getTech() == 9 && mana < 60){
                ...
                }if (player.getCapability(SlayerProvider.Breath_CAP, null).getBreath() > 2 || player.getCapability(SlayerProvider.Breath_CAP, null).getBreath() < 0 ) {
                    player.sendMessage(new TextComponentString("You Have To Learn ")
                            .appendSibling(new TextComponentString("Flame Breathing").setStyle(new Style().setColor(TextFormatting.DARK_RED)))
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int i, boolean B) {
        ...
        if(tick==20) {
            if (entity.onGround) {
                entity.motionX += 6.8D * look.x;
                entity.motionZ += 6.8D * look.z;
            } else {
                entity.motionX += 1.35D * look.x;
                entity.motionZ += 1.35D * look.z;
            }
        }
        if (this.tick > 0) {
            this.tick--;
