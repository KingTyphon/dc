...
public class idaten extends abs_idaten{
    public int explosionPower = 1;

    public idaten(World worldIn) {
        super(worldIn);
    }

    @SideOnly(Side.CLIENT)
    public idaten(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn, x, y, z, accelX, accelY, accelZ);
    }

    public idaten(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.getEntityWorld().isRemote)
            if (!(result.entityHit instanceof net.minecraft.entity.player.EntityPlayer) &&
                    !(result.entityHit instanceof idaten) &&
