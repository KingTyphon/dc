package net.entity.projectiles.Wind;
...
public abstract class abs_sinato extends Entity
{
    ...
    public abs_sinato(World worldIn) {
        super(worldIn);
    }
    ...
    public abs_sinato(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn);
        setSize(4.0F, 4.0F);
        setPosition(x, y, z);
    }

    public abs_sinato(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
        super(worldIn);
    ...
    public void onUpdate() {
        ...
        tick_living++;
        if(tick_living==10)setDead();

        if (this.inGround) {

            this.world.createExplosion(this, this.posX, this.posY, this.posZ, 2 * 0.9F, true);
        ...
        if (this.world.isRemote || ((this.shootingEntity == null || !this.shootingEntity.isDead) && this.world.isBlockLoaded(new BlockPos(this)))) {
            ...
            this.world.spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX+(0.3 * r.nextInt(5)), this.posY +(0.1 * r.nextInt(5)), this.posZ+(0.3 * r.nextInt(5)), 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX-(0.2 * r.nextInt(5)), this.posY -(0.1 * r.nextInt(5)), this.posZ-(0.2 * r.nextInt(5)), 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX+(0.1 * r.nextInt(5)), this.posY +(0.1 * r.nextInt(5)), this.posZ+(0.1 * r.nextInt(5)), 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX-(0.2 * r.nextInt(5)), this.posY -(0.1 * r.nextInt(5)), this.posZ-(0.5 * r.nextInt(5)), 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.SWEEP_ATTACK, this.posX-(0.2 * r.nextInt(5)), this.posY -(0.1 * r.nextInt(5)), this.posZ-(0.1 * r.nextInt(5)), 0.0D, 0.0D, 0.0D, new int[0]);
