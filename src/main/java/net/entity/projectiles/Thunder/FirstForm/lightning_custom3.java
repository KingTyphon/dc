...
public class lightning_custom3 extends abs_lightning2 {
    public int explosionPower = 1;
    public long boltVertex;

    public lightning_custom3(World worldIn) {
        super(worldIn);
    ...
    public lightning_custom3(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn, x, y, z, accelX, accelY, accelZ);
    }

    public lightning_custom3(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.getEntityWorld().isRemote)
            if (!(result.entityHit == this.shootingEntity) &&
                    !(result.entityHit instanceof lightning_custom3) &&
