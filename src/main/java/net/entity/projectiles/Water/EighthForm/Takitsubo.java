...
import net.entity.projectiles.Water.EighthForm.abs_takitsubo;
...
public class Takitsubo extends abs_takitsubo {
    public int explosionPower = 1;

    public Takitsubo(World worldIn) {
        super(worldIn);
    }

    @SideOnly(Side.CLIENT)
    public Takitsubo(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn, x, y, z, accelX, accelY, accelZ);
    }

    public Takitsubo(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.getEntityWorld().isRemote)
            if (!(result.entityHit instanceof net.minecraft.entity.player.EntityPlayer) &&
                    !(result.entityHit instanceof Takitsubo) &&
