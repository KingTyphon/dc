...
public class enko_head extends abs_head_enko {
    public int explosionPower = 1;

    public enko_head(World worldIn) {
        super(worldIn);
    }

    @SideOnly(Side.CLIENT)
    public enko_head(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn, x, y, z, accelX, accelY, accelZ);
    }

    public enko_head(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.getEntityWorld().isRemote)
            if (!(result.entityHit instanceof net.minecraft.entity.player.EntityPlayer) &&
                    !(result.entityHit instanceof enko_head) &&
