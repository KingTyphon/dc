...
public abstract class abs_rengoku extends Entity
{
    ...
    public abs_rengoku(World worldIn) {
        super(worldIn);
    }
    ...
    public abs_rengoku(World worldIn, double x, double y, double z, double accelX, double accelY, double accelZ) {
        super(worldIn);
        setSize(4.0F, 4.0F);
        setPosition(x, y, z);
    }

    public abs_rengoku(World worldIn, EntityLivingBase shooter, double accelX, double accelY, double accelZ) {
        super(worldIn);
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote || ((this.shootingEntity == null || !this.shootingEntity.isDead) && this.world.isBlockLoaded(new BlockPos(this)))) {
            ...
            Random r = new Random();
            float offset = r.nextFloat()-0.5f;
            float offset2= r.nextFloat()-0.5f;
            float offset3= r.nextFloat();
            if(offset3<0.05)this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX + offset , this.posY + 0.1D, this.posZ- offset, 0.0D, 0.0D, 0.0D, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX + offset2 , this.posY + 0.1D, this.posZ- offset2, 0.0D, 0.0D, 0.0D, new int[0]);
