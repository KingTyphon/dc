package net.entity.render.thunder;
...
public class Render_Lbolt extends Render<customLightning>
{
    public Render_Lbolt(RenderManager renderManagerIn)
    {
     ...
     */
    public void doRender(customLightning entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        GL11.glRotated(entityYaw, 0, 90,0 ); //pitch
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                for (int i1 = k; i1 >= l; --i1)
                {
                    ...
                    for (int j1 = 0; j1 < 5; ++j1)
                    {
                        ...
                        bufferbuilder.pos(d10 + d2, y + (double)(i1 ), d11 + d3).color(0.65F, 0.65F, 0.2F, 0.3F).endVertex();
                        bufferbuilder.pos(d8 + d4, y + (double)((i1 + 1)), d9 + d5).color(0.45F, 0.45F, 0.2F, 0.3F).endVertex();
                    }
     ...
     */
    @Nullable
    protected ResourceLocation getEntityTexture(customLightning entity)
    {
        return null;
    }
}